...
class _CropSelectionState extends State<CropSelection> {
  ...
  final ApiService apiService = ApiService('http://192.168.1.10:8050'); // Base URL for the backend
  ...
  // Request camera permissions and open camera for the selected crop
  Future<void> _requestCameraPermission(Crop selectedCrop) async {
    final status = await Permission.camera.request();
    if (status.isGranted) {
      _openCamera(context, selectedCrop);
  ...
  // Open camera for taking a picture of the selected crop
  Future<void> _openCamera(BuildContext context, Crop selectedCrop) async {
    if (!isCameraInitialized || cameras.isEmpty) {
      print("No camera available");
      return;
...
class _CameraPreviewScreenState extends State<CameraPreviewScreen> {
  ...
  final ApiService apiService = ApiService('http://192.168.1.10:8050'); // Base URL for backend
  ...
  Future<void> _takePicture() async {
    ...
    try {
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) => DisplayPictureScreen(
            imagePath: newImage.path, // Pass the path of the new image
            diseaseName: diseaseName, // Pass the disease name
            originalImage: imagePath, // Pass the original image path
          ),
        ),
      );
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(LocaleKeys.Take_a_Picture.tr(), style: const TextStyle(color: Colors.white)),
        backgroundColor: Colors.green,
      ),
      body: Column(
        children: [
